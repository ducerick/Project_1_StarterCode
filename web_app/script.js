...
async function getTotalOwed(user) {
	...
	for (var i = 0; i < users.length; i++) {
		total += await lookup(user, users[i]);
	}
	return total;
}
...
async function getLastActive(user) {
	var functioncall = getAllFunctionCalls(contractAddress, 'add_IOU');
	console.log(functioncall);
	var time = 0;
	var num = (await functioncall).length;
	var activity;
	for (var i = 0; i < num; i++) {
		if ((await functioncall)[i].from == user.toLowerCase() || (await functioncall)[i].args[0] == user.toLowerCase()) {
			activity = (await functioncall)[i].t;
			break;
		}
	}
	return activity;
...
async function getNeighbors(user) {
	...
	var users = await getUsers();
	for (var i = 0; i < users.length; i++) {
		var owed = await lookup(user, users[i]);
		if (owed > 0) neighbors.push(users[i]);
	}
	return neighbors;
}
...
async function add_IOU(creditor, amount, account) {
	//creditor is owned , account is debtor 
	var users = await getUsers();
	var start = users[0];
	const signer = await provider.getSigner(account);
	await BlockchainSplitwise.connect(signer).add_IOU(creditor, amount);

	var path = await doBFS(creditor, account, getNeighbors);
	console.log(path);
	var min=0;
	if (path != null) {
		var number = await path.length;
		min = amount;
		for (var i = 0; i < number - 1; i++) {
			var amount2 = await lookup(path[i], path[i + 1]);
			if (amount2 < min) {
				min = amount2;
			}
		}
		for( var i = 0; i<number - 1 ; i++ )
			{
				 await BlockchainSplitwise.add_IOU_sub(path[i], path[i+1], min);
			}
		await BlockchainSplitwise.add_IOU_sub(account, creditor, min);
	}
...
async function doBFS(start, end, getNeighbors) {
	...
	while (queue.length > 0) {
		...
		// or shift() returns the removed array element ( first element )
...
$("#addiou").click(function () {
	...
	add_IOU($("#creditor").val(), $("#amount").val(), defaultAccount).then((response) => {
...
async function sanityCheck() {
	...
	var last_activity = await getLastActive(accounts[0]);
	console.log("last activity ", last_activity);
	var neighborshood = await getNeighbors(accounts[0]);
	console.log("neighborhood ", neighborshood);
	//var path = await add_IOU(accounts[0],accounts[0],)
